...
void tGuiReportCancelTransaction::ExeCommand(QDataStream &)
{
    ...
    ((MainModule*)link)->CancelAllOperations();
...
void tReportGuiGetListServerModels::ExeCommand(QDataStream &_in)
{
    ...
    if(!((MainModule*)link)->IsRequeryServerModel)
    {
        ...
        bool tr=((MainModule*)link)->GetIsTransaction();
        if(tr)
        {
            ...
            ((MainModule*)link)->CorrectLastSynch(true);
    ...
    {
        ((MainModule*)link)->IsRequeryServerModel=false;
...
void tUpdateMainLocal::ExeCommand(QDataStream &)
{
    ...
    ((MainModule*)link)->OnListFilesLocal();
    emit NextCommand();
...
void tGUIReportReceiveLoginsTable::ExeCommand(QDataStream &_in)
{
    ...
    ((MainModule*)link)->UpdateLoginsTable(block);
...
{
    l="tClientGuiCommand \ttGUIReportReceiveReadPermissions\t Отчет запроса списка разрешений на чтение моделей с сервера";
    log.Write(l);
    ...
    ((MainModule*)link)->UpdateModelRead(block);

    ((MainModule*)link)->InternalCallBuildingTree();

    emit EndCommand();

}
//************************************************************************************************
void tGUISavePermissions::ExeCommand(QDataStream &_in)
{
    l="tClientGuiCommand \ttGUISavePermissions\t Передача таблицы разрешений на сервер ";
    log.Write(l);
...
//************************************************************************************************
void tGUIReportSavePermissions::ExeCommand(QDataStream &)
{
