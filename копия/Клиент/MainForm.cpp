...
MainForm::MainForm(QWidget *parent) : ui(new Ui::MainForm), QDialog(parent),conv(NULL), db_op(NULL), sLM_loc_list_models(NULL), slm_server_list_models(NULL), slm_list(NULL)
{
...
//    connect(ui->lvListFiles, SIGNAL(doubleClicked(QModelIndex)), this, SLOT(OnClickListFiles(QModelIndex)));
    ...
    connect(conv, SIGNAL(CloseMain()), this, SLOT(close()));
    ...
    conv->StartServer(ui->leAddr->text(), ui->sbPort->value());
...
void MainForm::OnConnect()
{
    ...
    conv->StartServer(S, my_settings.GetServerPort());
